...
function GestaoProfissionais() {
  ...
  const deletarProfissional = async () => {
    setLoading(true)
    try {
      const { error } = await db.delete('profissionais', {
        id: profissionalSelecionado
      })
      
      if (error) throw error
      mostrarMensagem('sucesso', 'Profissional removido com sucesso!')
      
      setModalDeletar(false)
      await carregarProfissionais()
      setProfissionalSelecionado(null)
    } catch (error) {
      mostrarMensagem('erro', 'Erro ao deletar: ' + error.message)
      setModalDeletar(false)
    } finally {
      setLoading(false)
    }
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                Você está prestes a remover o profissional:
              </p>
              <div style={{
                background: '#fff5f5',
            ...
            <div style={{
              ...
              <button
                ...
                onMouseEnter={(e) => {
                  if (!loading) e.target.style.background = '#cbd5e0'
                }}
                onMouseLeave={(e) => {
